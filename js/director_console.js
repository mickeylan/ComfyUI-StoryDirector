...
async function sendToMiniMaxDirector(plan){const targets=(app.graph?._nodes||[]).filter(node=>node.type==="MiniMaxH3DirectorCS");if(!targets.length){alert("画布中没有 MiniMaxH3DirectorCS 节点");return}let target=targets[0];if(targets.length>1){const choices=targets.map((node,index)=>`${index+1}. ${node.title||node.type}（节点 ${node.id}）`).join("\n");const selected=prompt(`选择要接收时间线的导演台：\n${choices}`,"1");if(selected===null)return;const index=Number(selected)-1;if(!Number.isInteger(index)||!targets[index]){alert("导演台序号无效");return}target=targets[index]}const editor=target._timelineEditor;if(!editor||typeof editor._applyLoadedTimeline!=="function"){alert("导演台尚未初始化，请刷新页面后重试");return}const response=await api.fetchApi("/story-director/last-processed");const latest=await response.json();if(!latest.timeline_data){alert("请先运行 StoryDirector 生成时间线");return}const timeline=JSON.parse(latest.timeline_data);timeline.global_prompt=plan.global_prompt||latest.global_prompt||"";timeline.overall_soundscape=plan.overall_soundscape||"";timeline.non_diegetic_music=plan.non_diegetic_music||"N/A";if(Array.isArray(plan.segments)&&plan.segments.length){if(plan.segments.length!==timeline.segments?.length){alert("当前编辑分镜数量与最近生成时间线不一致");return}timeline.segments.forEach((segment,index)=>segment.prompt=plan.segments[index].prompt||"")}if(!confirm(`将覆盖“${target.title||target.type}”当前时间线，是否继续？`))return;editor._applyLoadedTimeline(JSON.stringify(timeline),null)}
async function sendLatestToMiniMaxDirector(){const response=await api.fetchApi("/story-director/last-processed");const latest=await response.json();if(!latest.global_prompt||!latest.timeline_data){alert("请先运行 StoryDirector 生成时间线");return}const timeline=JSON.parse(latest.timeline_data);const plan={global_prompt:latest.global_prompt,overall_soundscape:timeline.overall_soundscape||"",non_diegetic_music:timeline.non_diegetic_music||"N/A",segments:(timeline.segments||[]).map(segment=>({prompt:segment.prompt||""}))};await sendToMiniMaxDirector(plan)}
async function renderDirectorResult(n,p){let plan={global_prompt:"",overall_soundscape:"",non_diegetic_music:"N/A",segments:[]},selected=0;try{if(get(n,"prompt_override"))plan=JSON.parse(get(n,"prompt_override"))}catch{}const savePlan=()=>set(n,"prompt_override",JSON.stringify(plan,null,2));const global=row(p,"总提示词",plan.global_prompt||"",v=>{plan.global_prompt=v;savePlan()},null,true);global.rows=12;const sound=row(p,"整体声音",plan.overall_soundscape||"",v=>{plan.overall_soundscape=v;savePlan()},null,true);sound.rows=3;const music=row(p,"非画面音乐",plan.non_diegetic_music||"N/A",v=>{plan.non_diegetic_music=v;savePlan()});const segment=E("textarea","width:100%;height:250px;box-sizing:border-box;margin-top:10px;background:#202329;color:#eee;border:1px solid #494f59;border-radius:4px;padding:8px");const status=E("span","color:#9db0c4;padding:7px");const show=()=>{selected=Math.min(selected,Math.max(0,(plan.segments||[]).length-1));segment.value=plan.segments?.[selected]?.prompt||"";status.textContent=plan.segments?.length?`第 ${selected+1}/${plan.segments.length} 镜`:"无分镜"};const apply=()=>{if(!plan.segments?.length)return;plan.segments[selected].prompt=segment.value;savePlan()};segment.onchange=apply;const bar=E("div","display:flex;gap:8px;align-items:center;flex-wrap:wrap");bar.append(B("发送到导演台",()=>{apply();sendToMiniMaxDirector(plan).catch(error=>alert(String(error.message||error)))}),B("读取最近结果",async()=>{const r=await api.fetchApi("/story-director/last-processed");const d=await r.json();if(!d.global_prompt||!d.timeline_data)return;const timeline=JSON.parse(d.timeline_data);plan={global_prompt:d.global_prompt,overall_soundscape:timeline.overall_soundscape||"",non_diegetic_music:timeline.non_diegetic_music||"N/A",segments:(timeline.segments||[]).map(x=>({prompt:x.prompt||""}))};global.value=plan.global_prompt;sound.value=plan.overall_soundscape;music.value=plan.non_diegetic_music;savePlan();show()}),B("◀",()=>{apply();selected=Math.max(0,selected-1);show()}),status,B("▶",()=>{apply();selected=Math.min((plan.segments?.length||1)-1,selected+1);show()}),B("复制本镜",()=>navigator.clipboard.writeText(segment.value||"")),B("复制 JSON",()=>navigator.clipboard.writeText(JSON.stringify(plan,null,2))),B("清空覆盖",()=>{plan={global_prompt:"",overall_soundscape:"",non_diegetic_music:"N/A",segments:[]};global.value="";sound.value="";music.value="N/A";segment.value="";set(n,"prompt_override","");show()}));p.append(bar,segment);show()}
function modal(n,id,title){document.querySelector(".storydirector-modal")?.remove();const s=Object.assign({assets:[],preference:{}},state(n));const o=E("div","position:fixed;inset:0;background:#000b;z-index:10000;display:flex;align-items:center;justify-content:center");o.className="storydirector-modal";const d=E("section","width:860px;max-height:90vh;background:#191b20;border:1px solid #464c56;border-radius:9px;display:flex;flex-direction:column;box-shadow:0 20px 70px #000");const h=E("header","display:flex;padding:17px 20px;border-bottom:1px solid #3a3f48");h.append(E("strong","font-size:18px;color:#eee;flex:1",title),B("✕",()=>o.remove()));const c=E("main","padding:16px 20px;overflow:auto;min-height:390px;color:#ddd");if(id==="assets")renderAssets(n,c,s);else if(id==="config")renderConfig(n,c);else if(id==="prefs")renderPrefs(n,c,s);else if(id==="result")renderDirectorResult(n,c);else c.innerHTML="<h3>使用步骤</h3><ol><li>在节点的故事内容框输入故事。</li><li>导入并描述参考素材。</li><li>配置拆解模式、本地 GGUF 和镜头偏好。</li><li>运行节点生成完整 H3 剧本。</li><li>在分段结果编辑中读取、修改或复制结果。</li></ol><p>本节点不编码、不采样、不解码、不保存视频。</p>";const f=E("footer","display:flex;justify-content:flex-end;padding:12px 20px;border-top:1px solid #3a3f48");f.append(B("完成",()=>o.remove()));d.append(h,c,f);o.append(d);document.body.append(o)}
...
function attachMentionMenu(node) {
  ...
  const choose=(asset)=>{const text=storyText(input),start=pos-match[0].length,value=text.slice(0,start)+referenceName(asset)+text.slice(pos);set(node,"story",value);renderStory(node,start+referenceName(asset).length);input.focus();close()};
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const sendButton=B("📤 发送到导演台",()=>sendLatestToMiniMaxDirector().catch(error=>alert(String(error.message||error))));
      sendButton.style.gridColumn="1 / -1";
