...
function configureUploadedAssets(assets) {
  return new Promise((resolve) => {
    ...
    assets.forEach((asset,index)=>{
      ...
      card.append(preview);
      const fields=E("div");
      const name=row(fields,"角色/素材名称",asset.name,value=>asset.name=value.trim());
      name.placeholder=asset.type==="image"?"例如：女主小雨 / 反派老周":asset.type==="video"?"例如：追逐参考 / 环绕运镜":"例如：女主音色 / 雨声音效";
      row(fields,"素材用途",asset.role,value=>asset.role=value,TYPES[asset.type]);
      row(fields,"外观/声音描述",asset.description||"",value=>asset.description=value.trim(),null,true);
      fields.append(E("div","color:#718096;font-size:11px",`${index+1}. ${asset.path}`));
...
function assetToken(node, asset) {
  ...
  if(asset.type==="image"){
    ...
    token.style.padding="2px 6px";token.append(E("span","font-size:18px",asset.type==="video"?"🎬":"🎧"));
  }
  return token;
}
