...
function insertAssetName(node, name) {
  ...
  const value = before + left + name + right + after;
  set(node, "story", value);
  if(input){input.value=value;const pos=(before+left+name).length;input.focus();input.setSelectionRange(pos,pos)}
}

function attachMentionMenu(node) {
  const input = W(node,"story")?.inputEl;
  if(!input || input.__storyDirectorMentions) return;
  input.__storyDirectorMentions=true;
  const menu=E("div","position:fixed;z-index:11000;display:none;max-height:260px;overflow:auto;background:#202329;border:1px solid #566171;border-radius:6px;box-shadow:0 8px 25px #000;padding:4px");document.body.append(menu);
  const close=()=>menu.style.display="none";
  input.addEventListener("input",()=>{const pos=input.selectionStart??input.value.length;const prefix=input.value.slice(0,pos);const match=prefix.match(/@([^\s@]*)$/);if(!match){close();return}const query=match[1].toLowerCase();const assets=(state(node).assets||[]).filter(a=>a.enabled!==false&&String(a.name||"").toLowerCase().includes(query));menu.replaceChildren();if(!assets.length){close();return}const rect=input.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${Math.min(window.innerHeight-280,rect.bottom+4)}px`;menu.style.width=`${Math.min(360,rect.width)}px`;menu.style.display="block";assets.forEach(a=>{const item=B(`${a.type==="image"?"🖼️":a.type==="video"?"🎬":"🎧"} ${a.name}`,()=>{const value=input.value.slice(0,pos-match[0].length)+a.name+input.value.slice(pos);set(node,"story",value);input.value=value;const next=pos-match[0].length+a.name.length;input.focus();input.setSelectionRange(next,next);close()});item.style.cssText+=";display:block;width:100%;text-align:left;margin:2px 0";menu.append(item)})});input.addEventListener("blur",()=>setTimeout(close,150));
}
