...
function assetToken(node, asset) {
  ...
  const token=E("span",`display:inline-flex;align-items:center;gap:3px;background:${colors[asset.type]};color:#fff;border-radius:4px;padding:1px 5px;margin:0 2px;white-space:nowrap;vertical-align:middle`,`${asset.type==="image"?"🖼️":asset.type==="video"?"🎬":"🎧"} ${asset.name}`);
  token.contentEditable="false";token.dataset.assetName=asset.name;token.title=asset.description||asset.path||"";return token;
}
...
function attachMentionMenu(node) {
  ...
  const paint=()=>items.forEach((x,i)=>x.button.style.background=i===active?"#315d78":"#292d35");
  input.addEventListener("input",()=>{const text=storyText(input);set(node,"story",text);pos=caretOffset(input);match=text.slice(0,pos).match(/@([^\s@]*)$/);if(!match){renderStory(node,pos);close();return}const query=match[1].toLowerCase(),assets=(state(node).assets||[]).filter(a=>a.enabled!==false&&String(a.name||"").toLowerCase().includes(query));menu.replaceChildren();items=[];active=0;if(!assets.length){close();return}const rect=input.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${Math.min(innerHeight-280,rect.bottom+4)}px`;menu.style.width=`${Math.min(380,rect.width)}px`;menu.style.display="block";assets.forEach(asset=>{const b=B(`${asset.type==="image"?"🖼️":asset.type==="video"?"🎬":"🎧"} ${asset.name}`,()=>choose(asset));b.style.cssText+=";display:block;width:100%;text-align:left;margin:2px 0";menu.append(b);items.push({asset,button:b})});paint()});
  input.addEventListener("keydown",e=>{if(!items.length)return;if(e.key==="ArrowDown"||e.key==="ArrowUp"){e.preventDefault();active=(active+(e.key==="ArrowDown"?1:-1)+items.length)%items.length;paint()}else if(e.key==="Enter"||e.key==="Tab"){e.preventDefault();choose(items[active].asset)}else if(e.key==="Escape"){e.preventDefault();close()}});input.addEventListener("keyup",()=>node.__storyCaret=caretOffset(input));input.addEventListener("mouseup",()=>node.__storyCaret=caretOffset(input));input.addEventListener("blur",()=>setTimeout(close,150));
}
