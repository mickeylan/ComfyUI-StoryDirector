...
function captureWorkflowState(node){clearTimeout(node.__storyDirectorChangeTimer);node.graph?.setDirtyCanvas(true,true);const tracker=app.extensionManager?.workflow?.activeWorkflow?.changeTracker||app.workflowManager?.activeWorkflow?.changeTracker,before=tracker?.activeState?.nodes?.find(item=>String(item.id)===String(node.id))?.widgets_values?.[0];if(tracker?.checkState)tracker.checkState();else window.dispatchEvent(new MouseEvent("mouseup",{bubbles:true,cancelable:true}));const after=tracker?.activeState?.nodes?.find(item=>String(item.id)===String(node.id))?.widgets_values?.[0];console.info(`[StoryDirector] 工作流状态同步 node=${node.id} tracker=${Boolean(tracker?.checkState)} widgetLength=${String(get(node,"story")||"").length} beforeLength=${typeof before==="string"?before.length:"null"} afterLength=${typeof after==="string"?after.length:"null"} updated=${before!==after}`)}
function notifyWorkflowChanged(node){clearTimeout(node.__storyDirectorChangeTimer);node.__storyDirectorChangeTimer=setTimeout(()=>captureWorkflowState(node),250)}
const setWidget=(n,k,v)=>{const w=W(n,k);if(w){w.value=v;w.callback?.(v)}return Boolean(w)};
const set=(n,k,v)=>{setWidget(n,k,v);notifyWorkflowChanged(n)};
...
const referenceName=a=>String(a?.reference_name||a?.name||"").trim();
const setStory=(node,value)=>{const s=state(node),names=(s.assets||[]).map(referenceName).filter(Boolean);s.story_text=value;s.prompt_override="";s.asset_references=names.filter(name=>value.includes(`@${name}`));const storyWidget=setWidget(node,"story",value),stateWidget=setWidget(node,"director_state",JSON.stringify(s));setWidget(node,"prompt_override","");console.info(`[StoryDirector] 故事输入已同步 node=${node.id} length=${value.length} references=${s.asset_references.length} storyWidget=${storyWidget} stateWidget=${stateWidget}`);captureWorkflowState(node)};
...
function editAssetMetadata(node, asset) {
  const draft={...asset};
  ...
  const dialog=E("section","width:620px;background:#191b20;border:1px solid #4b5563;border-radius:9px;box-shadow:0 20px 70px #000");
  const head=E("header","padding:16px 20px;border-bottom:1px solid #3b424d"); head.append(E("strong","font-size:18px;color:#eee","修改素材信息"));
  const content=E("main","padding:16px 20px");
  row(content,"引用名称（@使用）",referenceName(draft),value=>draft.reference_name=value.trim());
  row(content,"素材用途",draft.role||"其他",value=>draft.role=value,TYPES[draft.type]);
  row(content,"详细描述（给 LLM）",draft.description||"",value=>draft.description=value.trim(),null,true);
  const footer=E("footer","display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid #3b424d");
  footer.append(B("取消",()=>overlay.remove()),B("保存修改",()=>{const name=referenceName(draft);if(!name){alert("引用名称不能为空");return}const old=referenceName(asset);Object.assign(asset,draft,{name,reference_name:name});const current=state(node);const target=(current.assets||[]).find(item=>item.path===asset.path);if(target)Object.assign(target,asset);if(old&&old!==name)setStory(node,String(get(node,"story")||"").split(`@${old}`).join(`@${name}`));save(node,current);node.__storyDirectorRenderStory?.();overlay.remove()}));
  dialog.append(head,content,footer);overlay.append(dialog);document.body.append(overlay);
}
function configureUploadedAssets(assets) {
  return new Promise((resolve) => {
    ...
    assets.forEach((asset,index)=>{
      ...
      card.append(preview);
      const fields=E("div");
      const name=row(fields,"引用名称（@使用）",asset.reference_name,value=>asset.reference_name=value.trim());
...
function renderAssets(n,p,s){s.assets??=[];const tools=E("div","display:flex;gap:8px;margin-bottom:12px");const list=E("div");const draw=()=>{list.replaceChildren();for(const kind of ["image","video","audio"]){const items=s.assets.map((a,i)=>[a,i]).filter(([a])=>a.type===kind);list.append(E("h3","color:#8fc7e8;border-bottom:1px solid #3b424d;padding-bottom:5px",`${kind==="image"?"🖼️ 图片":kind==="video"?"🎬 视频":"🎧 音频"}（${items.length}）`));items.forEach(([a,i])=>{const card=E("div","display:grid;grid-template-columns:145px 1fr 70px;gap:9px;background:#202329;border:1px solid #414955;border-radius:7px;padding:9px;margin:7px 0");const pv=E("div","height:100px;background:#111;display:flex;align-items:center;justify-content:center;overflow:hidden");const m=document.createElement(kind==="image"?"img":kind==="video"?"video":"audio");m.src=api.apiURL(`/story-director/preview?path=${encodeURIComponent(a.path||"")}`);m.style.cssText="max-width:100%;max-height:100%";if(kind!=="image")m.controls=true;pv.append(m);card.append(pv);const f=E("div");row(f,"引用名称（@使用）",referenceName(a),v=>{const old=referenceName(a);a.reference_name=v;a.name=v;if(old&&old!==v){const text=String(get(n,"story")||"").split(`@${old}`).join(`@${v}`);setStory(n,text);n.__storyDirectorRenderStory?.()}save(n,s)});row(f,"类型",a.role||"其他",v=>{a.role=v;save(n,s)},TYPES[kind]);row(f,"详细描述（给 LLM）",a.description||"",v=>{a.description=v;save(n,s)},null,true);f.append(E("small","color:#7896ae",a.path||""));card.append(f);const ac=E("div","display:flex;flex-direction:column;gap:5px");const en=document.createElement("input");en.type="checkbox";en.checked=a.enabled!==false;en.onchange=()=>{a.enabled=en.checked;save(n,s);draw()};ac.append(en,B("↑",()=>{if(i)[s.assets[i-1],s.assets[i]]=[s.assets[i],s.assets[i-1]];save(n,s);draw()}),B("↓",()=>{if(i<s.assets.length-1)[s.assets[i+1],s.assets[i]]=[s.assets[i],s.assets[i+1]];save(n,s);draw()}),B("删除",async()=>{if(!confirm("删除该素材及 input/story_director 中的副本？"))return;await api.fetchApi("/story-director/delete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:a.path,remove_file:true})});s.assets.splice(i,1);save(n,s);draw()}));card.append(ac);list.append(card)})}const intro=s.assets.filter(a=>a.enabled!==false).map((a,i)=>`${a.type==="image"?"图片":a.type==="video"?"视频":"音频"}${i+1} = ${referenceName(a)}${a.description?`（${a.description}）`:""}`).join("\n");const ta=row(list,"传给 LLM 的素材描述",intro,()=>{},null,true);ta.readOnly=true};tools.append(B("➕ 导入素材",()=>{const x=document.createElement("input");x.type="file";x.multiple=true;x.accept="image/*,video/*,audio/*";x.onchange=async()=>{const uploaded=[];for(const file of x.files){const fd=new FormData();fd.append("file",file,file.name);const r=await api.fetchApi("/story-director/upload",{method:"POST",body:fd});const d=await r.json();if(!r.ok)alert(d.error||"导入失败");else uploaded.push(...(d.assets||[]))}if(uploaded.length){const configured=await configureUploadedAssets(uploaded);if(configured)s.assets.push(...configured)}save(n,s);draw()};x.click()}),B("导出目录",()=>downloadJson("story-director-assets.json",{assets:s.assets})),B("导入目录",()=>{const x=document.createElement("input");x.type="file";x.accept=".json,application/json";x.onchange=async()=>{try{const raw=JSON.parse(await x.files[0].text());const r=await api.fetchApi("/story-director/import",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({catalog:raw})});const d=await r.json();if(!r.ok)throw new Error(d.error||"导入失败");const flat=[];for(const [key,kind] of [["images","image"],["videos","video"],["audios","audio"]])for(const a of d.catalog?.[key]||[])flat.push({...a,type:kind,role:a.type||"其他"});s.assets=flat;save(n,s);draw()}catch(e){alert(String(e.message||e))}};x.click()}),B("清空列表",()=>{if(confirm("只清空列表，保留托管文件？")){s.assets=[];save(n,s);draw()}}));p.append(tools,list);draw()}
function renderConfig(n,p,s){row(p,"运行模式",get(n,"mode"),v=>set(n,"mode",v),["拆解模式 (Decompose)","生成模式 (Generate)","离线预览"]);row(p,"导演 Skill",get(n,"director_skill"),v=>set(n,"director_skill",v),Array.from(W(n,"director_skill")?.options?.values||[]));for(const [label,key] of [["故事风格","story_style"],["分段数量","segment_count"],["Qwen GGUF（3.5 / 3.8）","llm_model"],["Qwen mmproj（配套版本）","llm_mmproj"]])row(p,label,get(n,key),v=>set(n,key,v),Array.from(W(n,key)?.options?.values||[]));row(p,"每段时长（秒）",get(n,"segment_duration"),v=>set(n,"segment_duration",Number(v)));row(p,"输出语言",get(n,"prompt_lang"),v=>set(n,"prompt_lang",v),["中文 [ZH]","英文 [EN]"]);row(p,"自定义生成规则",get(n,"custom_rules"),v=>set(n,"custom_rules",v),null,true);row(p,"拆解后二次增强",get(n,"enhance")?"开启":"关闭",v=>set(n,"enhance",v==="开启"),["关闭","开启"]);p.append(E("h3","color:#8fc7e8;margin-top:20px","本地 LLM（llama-cpp-python）"));row(p,"上下文长度",get(n,"context_size"),v=>set(n,"context_size",Number(v)));row(p,"GPU 层数（-1=全部）",get(n,"gpu_layers"),v=>set(n,"gpu_layers",Number(v)));row(p,"最大输出 tokens",get(n,"max_tokens"),v=>set(n,"max_tokens",Number(v)));row(p,"temperature",get(n,"temperature"),v=>set(n,"temperature",Number(v)));row(p,"top_k",get(n,"top_k"),v=>set(n,"top_k",Number(v)));row(p,"top_p",get(n,"top_p"),v=>set(n,"top_p",Number(v)));row(p,"min_p",get(n,"min_p"),v=>set(n,"min_p",Number(v)));row(p,"repeat_penalty",get(n,"repeat_penalty"),v=>set(n,"repeat_penalty",Number(v)));row(p,"随机种子",get(n,"seed"),v=>set(n,"seed",Number(v)));s.qwen38??={mtp:true,mtp_draft_tokens:2,reasoning_effort:"xhigh",cpu_moe:false,n_cpu_moe:0};p.append(E("h3","color:#8fc7e8;margin-top:20px","Qwen3.8 专用"));row(p,"MTP",s.qwen38.mtp!==false?"开启":"关闭",v=>{s.qwen38.mtp=v==="开启";save(n,s)},["开启","关闭"]);row(p,"MTP draft tokens",s.qwen38.mtp_draft_tokens??2,v=>{s.qwen38.mtp_draft_tokens=Math.max(1,Math.min(8,Number(v)||2));save(n,s)});row(p,"推理强度",s.qwen38.reasoning_effort||"xhigh",v=>{s.qwen38.reasoning_effort=v;save(n,s)},["xhigh","medium","low"]);row(p,"全部 MoE 专家转 CPU",s.qwen38.cpu_moe?"开启":"关闭",v=>{s.qwen38.cpu_moe=v==="开启";save(n,s)},["关闭","开启"]);row(p,"前 N 层 MoE 转 CPU",s.qwen38.n_cpu_moe??0,v=>{s.qwen38.n_cpu_moe=Math.max(0,Number(v)||0);save(n,s)})}
...
async function renderDirectorResult(n,p){let plan={global_prompt:"",overall_soundscape:"",non_diegetic_music:"N/A",segments:[]},selected=0;const strategy=E("div","margin-bottom:12px;padding:9px;background:#202832;border:1px solid #405368;border-radius:5px;color:#9ec9e5","本次导演策略：尚未读取结果");p.append(strategy);try{if(get(n,"prompt_override"))plan=JSON.parse(get(n,"prompt_override"))}catch{}const savePlan=()=>{const value=JSON.stringify(plan,null,2);set(n,"prompt_override",value);n.properties??={};n.properties.storydirector_prompt_override=value};const global=row(p,"总提示词",plan.global_prompt||"",v=>{plan.global_prompt=v;savePlan()},null,true);global.rows=12;const sound=row(p,"整体声音",plan.overall_soundscape||"",v=>{plan.overall_soundscape=v;savePlan()},null,true);sound.rows=3;const music=row(p,"非画面音乐",plan.non_diegetic_music||"N/A",v=>{plan.non_diegetic_music=v;savePlan()});const segment=E("textarea","width:100%;height:250px;box-sizing:border-box;margin-top:10px;background:#202329;color:#eee;border:1px solid #494f59;border-radius:4px;padding:8px");const status=E("span","color:#9db0c4;padding:7px");const show=()=>{selected=Math.min(selected,Math.max(0,(plan.segments||[]).length-1));segment.value=plan.segments?.[selected]?.prompt||"";status.textContent=plan.segments?.length?`第 ${selected+1}/${plan.segments.length} 镜`:"无分镜"};const apply=()=>{if(!plan.segments?.length)return;plan.segments[selected].prompt=segment.value;savePlan()};segment.oninput=apply;const bar=E("div","display:flex;gap:8px;align-items:center;flex-wrap:wrap");bar.append(B("读取最近结果",async()=>{const r=await api.fetchApi("/story-director/last-processed");const d=await r.json();if(!d.global_prompt||!d.timeline_data)return;const timeline=JSON.parse(d.timeline_data);strategy.textContent=`本次导演策略：${timeline.selected_skill||"通用 H3 提示词"}\n选择原因：${timeline.skill_selection_reason||"未记录"}`;strategy.style.whiteSpace="pre-wrap";plan={global_prompt:d.global_prompt,overall_soundscape:timeline.overall_soundscape||"",non_diegetic_music:timeline.non_diegetic_music||"N/A",segments:(timeline.segments||[]).map(x=>({prompt:x.prompt||""}))};global.value=plan.global_prompt;sound.value=plan.overall_soundscape;music.value=plan.non_diegetic_music;savePlan();show()}),B("◀",()=>{apply();selected=Math.max(0,selected-1);show()}),status,B("▶",()=>{apply();selected=Math.min((plan.segments?.length||1)-1,selected+1);show()}),B("复制本镜",()=>navigator.clipboard.writeText(segment.value||"")),B("复制 JSON",()=>navigator.clipboard.writeText(JSON.stringify(plan,null,2))),B("清空覆盖",()=>{plan={global_prompt:"",overall_soundscape:"",non_diegetic_music:"N/A",segments:[]};global.value="";sound.value="";music.value="N/A";segment.value="";set(n,"prompt_override","");show()}));p.append(bar,segment);show()}
function modal(n,id,title){document.querySelector(".storydirector-modal")?.remove();const s=Object.assign({assets:[],preference:{}},state(n));const o=E("div","position:fixed;inset:0;background:#000b;z-index:10000;display:flex;align-items:center;justify-content:center");o.className="storydirector-modal";const d=E("section","width:860px;max-height:90vh;background:#191b20;border:1px solid #464c56;border-radius:9px;display:flex;flex-direction:column;box-shadow:0 20px 70px #000");const h=E("header","display:flex;padding:17px 20px;border-bottom:1px solid #3a3f48");h.append(E("strong","font-size:18px;color:#eee;flex:1",title),B("✕",()=>o.remove()));const c=E("main","padding:16px 20px;overflow:auto;min-height:390px;color:#ddd");if(id==="assets")renderAssets(n,c,s);else if(id==="config")renderConfig(n,c,s);else if(id==="prefs")renderPrefs(n,c,s);else if(id==="result")renderDirectorResult(n,c);else c.innerHTML="<h3>使用步骤</h3><ol><li>在节点的故事内容框输入故事。</li><li>导入并描述参考素材。</li><li>配置拆解模式、本地 GGUF 和镜头偏好。</li><li>运行节点生成完整 H3 剧本。</li><li>在分段结果编辑中读取、修改或复制结果。</li></ol><p>本节点不编码、不采样、不解码、不保存视频。</p>";const f=E("footer","display:flex;justify-content:flex-end;padding:12px 20px;border-top:1px solid #3a3f48");f.append(B("完成",()=>o.remove()));d.append(h,c,f);o.append(d);document.body.append(o)}
function storyText(editor) {
  let text="";
  editor.childNodes.forEach(child=>{text+=child.nodeType===Node.TEXT_NODE?child.textContent:(child.dataset?.assetName?`@${child.dataset.assetName}`:child.textContent);});
...
function assetToken(node, asset) {
  ...
  token.contentEditable="false";token.dataset.assetName=referenceName(asset);token.title=[referenceName(asset),asset.description].filter(Boolean).join("\n");
  if(asset.type==="image"){
  ...
  token.append(E("span","padding:2px 6px;font-size:12px;max-width:130px;overflow:hidden;text-overflow:ellipsis",referenceName(asset)));
  return token;
}
function renderStory(node, caret=null) {
  const editor=node.__storyEditor;if(!editor||node.graph!==app.graph)return;const text=String(get(node,"story")||"");const assets=(state(node).assets||[]).filter(a=>a.enabled!==false&&referenceName(a)).sort((a,b)=>referenceName(b).length-referenceName(a).length);editor.replaceChildren();let i=0,last=0;while(i<text.length){const hit=assets.find(a=>text.startsWith(`@${referenceName(a)}`,i));if(hit){if(i>last)editor.append(document.createTextNode(text.slice(last,i)));editor.append(assetToken(node,hit));i+=referenceName(hit).length+1;last=i}else i++}if(last<text.length)editor.append(document.createTextNode(text.slice(last)));console.info(`[StoryDirector] 故事界面已渲染 node=${node.id} widgetLength=${text.length} domLength=${storyText(editor).length} connected=${editor.isConnected}`);if(caret!==null){const sel=getSelection(),range=document.createRange();let remain=caret;for(const child of editor.childNodes){const len=child.nodeType===Node.TEXT_NODE?child.textContent.length:(child.dataset?.assetName?child.dataset.assetName.length+1:0);if(remain<=len){range.setStartAfter(child);range.collapse(true);break}remain-=len}sel.removeAllRanges();sel.addRange(range)}}
...
function attachMentionMenu(node) {
  ...
  const choose=(asset)=>{const text=storyText(input),start=pos-match[0].length,mention=`@${referenceName(asset)}`,value=text.slice(0,start)+mention+text.slice(pos);setStory(node,value);renderStory(node,start+mention.length);input.focus();close()};
  ...
  input.addEventListener("input",()=>{const text=storyText(input);pos=caretOffset(input);match=text.slice(0,pos).match(/@([^\s@]*)$/);if(!match){close();return}const query=match[1].toLowerCase(),assets=(state(node).assets||[]).filter(a=>a.enabled!==false&&referenceName(a).toLowerCase().includes(query));menu.replaceChildren();items=[];active=0;if(!assets.length){close();return}const rect=input.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${Math.min(innerHeight-360,rect.bottom+4)}px`;menu.style.width="auto";menu.style.maxWidth="460px";menu.style.display="grid";menu.style.gridTemplateColumns=`repeat(${Math.min(5,Math.max(1,assets.length))},74px)`;menu.style.gap="6px";assets.forEach(asset=>{const b=E("button","width:74px;height:96px;padding:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;background:transparent;color:#ddd;border:1px solid transparent;border-radius:6px;cursor:pointer");b.type="button";b.title=[referenceName(asset),asset.description].filter(Boolean).join("\n");if(asset.type==="image"){const image=document.createElement("img");image.src=api.apiURL(`/story-director/preview?path=${encodeURIComponent(asset.path||"")}`);image.alt="";image.style.cssText="width:64px;height:58px;object-fit:cover;border-radius:4px;display:block";b.append(image)}else b.append(E("div","width:64px;height:58px;display:flex;align-items:center;justify-content:center;background:#111;border-radius:4px;font-size:28px",asset.type==="video"?"🎬":"🎧"));b.append(E("span","width:66px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px",referenceName(asset)));b.onclick=()=>choose(asset);b.onmousedown=e=>e.preventDefault();menu.append(b);items.push({asset,button:b})});paint()});
...
function createAssetWindow(node) {
  ...
  const render = () => {
    ...
    enabled.forEach((asset) => {
      ...
      card.type = "button";
      card.title = `点击插入素材名：${referenceName(asset)}\n${asset.description || ""}`;
      if (asset.type === "image") {
      ...
      card.append(E("div", "position:absolute;left:0;right:0;bottom:0;background:#111d;padding:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:11px", referenceName(asset) || asset.path));
      card.onclick = () => insertAssetName(node, referenceName(asset));
      const edit = B("修改", (event) => { event.stopPropagation(); editAssetMetadata(node, asset); });
      edit.style.cssText += ";position:absolute;top:3px;right:3px;padding:2px 5px;font-size:10px;background:#111c";
      card.append(edit);
      grid.append(card);
    });
  };
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const node = this;
      for (const name of ["prompt_override", "preference", "custom_rules", "enhance", "llm_model", "llm_mmproj", "context_size", "gpu_layers", "max_tokens", "temperature", "top_k", "top_p", "min_p", "repeat_penalty", "seed", "director_state", "director_skill"]) {
    ...
    nodeType.prototype.onConfigure = function () {
      ...
      console.info(`[StoryDirector] 工作流节点已恢复 node=${this.id} storyLength=${String(get(this,"story")||"").length} references=${state(this).asset_references?.length||0}`);
      setTimeout(() => {const s=state(this),saved=s.story_text;if(typeof saved==="string"&&get(this,"story")!==saved){setWidget(this,"story",saved);console.info(`[StoryDirector] 已阻止旧故事覆盖 node=${this.id} restoredLength=${saved.length}`)}this.__storyDirectorRenderStory?.();this.__storyDirectorRenderAssets?.()}, 0);
      return result;
    };
  }
});
