...
const set=(n,k,v)=>{const w=W(n,k);if(w){w.value=v;w.callback?.(v)}n.graph?.setDirtyCanvas(true,true);n.graph?.change?.()};
...
const setStory=(node,value)=>{set(node,"story",value);set(node,"prompt_override","");node.properties??={};node.properties.storydirector_story=value;node.properties.storydirector_prompt_override="";const s=state(node),names=(s.assets||[]).map(referenceName).filter(Boolean);s.story_text=value;s.prompt_override="";s.asset_references=names.filter(name=>value.includes(`@${name}`));save(node,s)};
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const node = this;
      for (const name of ["prompt_override", "preference", "custom_rules", "enhance", "llm_model", "llm_mmproj", "context_size", "gpu_layers", "max_tokens", "temperature", "top_k", "top_p", "min_p", "repeat_penalty", "seed", "director_state"]) {
