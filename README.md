...
1. 在 ComfyUI Python 环境安装带 MTMD 支持的 `llama-cpp-python`（仅选择本地模型时需要）。
2. 将配套的 Qwen3.5 GGUF 与 `mmproj` GGUF 放入 `models/LLM` 的同一模型目录，添加唯一的 `StoryDirector` 节点，并在“剧本拆解配置”中分别选择两者。当前只开放 Qwen3.5，Qwen3.8 待实际调试通过后再加入。
